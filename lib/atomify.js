...
function lookupMetadata(filename, callback) {
  ...
  if (data[filename].tv_show) {
    if((match = data[filename].show.match(/^(NCIS|Criminal Minds|Green Lantern|Avatar|Tron|DreamWorks Dragons|Primeval) (.*)/)) !== null){
    ...
    db = new sqlite3.Database(kutil.couchpotato_dir+'/couchpotato.db', sqlite3.OPEN_READONLY, function(err) {
